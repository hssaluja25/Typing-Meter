...
import typing_meter.signals
from typing_meter.calculations import (
    accuracy,
...
from typing_meter.commandline import load_from_database, resolve_commandline_arguments
from typing_meter.history import save_history
from typing_meter.keycheck import (
    is_backspace,
    is_ctrl_backspace,
    is_ctrl_c,
    is_ctrl_t,
...
from typing_meter.timer import get_elapsed_minutes_since_first_keypress


class App:
    """Class for enclosing all methods required to run typing_meter."""
    ...
    def setup_print(self, win):
        ...
        win.addstr(0, self.window_width // 2 - 4, " typing_meter ", self.Color.BLUE)
    ...
    def print_realtime_wpm(self, win):
        ...
        """
        current_wpm = 0
        total_time = typing_meter.timer.get_elapsed_minutes_since_first_keypress(
            self.start_time,
        )
        if total_time != 0:
