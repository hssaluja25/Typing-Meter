...
def resolve_commandline_arguments():
    ...
    if opt.history is not None:
        ...
        text, text_id = load_from_database(opt.id, "english")
...
def load_from_database(text_id, language):
    ...
    if 1 <= text_id <= row_count:
        if language == "english":
            text = database.fetch_text_from_id(text_id, "english")
            return text, text_id
        else:
            database.fetch_text_from_id(text_id, "hindi")
            return 
...
def load_based_on_difficulty(difficulty_level=random.randrange(1, 6)):
    ...
    if 1 <= difficulty_level <= max_level:
        ...
        text = database.fetch_text_from_id(text_id, "hindi")
